...
사용자가 "main에 반영", "메인에 반영", "메인으로 올려"처럼 main 브랜치 반영을 요청하면, 직접 main 브랜치에 커밋하거나 병합하지 않고 반드시 PR 기반으로 처리한다.

- 작업 브랜치에서 변경 사항을 커밋한다.
- 원격 작업 브랜치로 push한 뒤 main을 base로 하는 PR을 생성하거나 기존 PR을 업데이트한다.
- 사용자가 명시적으로 직접 main 병합/커밋을 요구한 경우에도 한 번 확인하고 진행한다.
- main 브랜치로 전환해 직접 merge, cherry-pick, commit, push 하지 않는다.
